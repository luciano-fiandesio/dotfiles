...
function hello() {
    ...
    echo "                                              "
    echo "           Brew + Fish + Fisher               "
    echo "                 by @ghaiklor                 "
    echo "                                              "
    echo "                                              "

    blue_color
    echo "This script will guide you through installing all the required dependencies for Fish Shell + Fisher"
...
function install_fisherman() {
    ...
    if [[ ! -f ~/.config/fish/functions/fisher.fish ]] ; then
        ...
        blue_color
        echo "You already have Fisher installed"
    fi
...
function install_prompt() {
    ...
    if brew ls --versions starship > /dev/null; then
        blue_color
        echo "You already have the Starship prompt installed"
    else
        blue_color
        echo "Install the Starship prompt (starship.rs)"

        green_color
        read -p "Do you want to install the Starship prompt (y/N) " -n 1 answer
        echo
        if [[ ${answer} == "y" || ${answer} == "Y" ]]; then
            blue_color
            ...
            blue_color
            echo "Skipping Starship prompt installation..."
        fi
